...
class TimesheetView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        ).values('work_entries_id', 'date', 'start_time', 'end_time', 'duration', 'task_type__name',
                 'customer__customer_name', 'task_id', 'task__task_name', 'project__project_name', 'phase__phase_name',
                 'sale', 'sale__sales_name', 'sales_task__task_name', 'description')

        absences = Absence.objects.filter(
            user=user, absence_date__range=[week_start, week_end]
        ).values('absence_id', 'absence_date', 'start_time', 'end_time', 'duration', 'leave_type', 'leave_type__name',
                 'project_id', 'project__project_name', 'sale', 'sale__sales_name', 'absence_description')

        expenses = Expense.objects.filter(
            user=user, date__range=[week_start, week_end]
        ).values('expense_id', 'date', 'value', 'description', 'task_id', 'task__task_name', 'customer__customer_name',
                 'project__project_name', 'phase__phase_name', 'sale', 'sale__sales_name', 'sales_task_id',
